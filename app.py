...
# Page Configuration
st.set_page_config(
    page_title="Elite Prep | Grade 6-9 Math & English Practice",
    page_icon="🎓",
    layout="wide",
...
def load_local_resources():
    ...
    # No external textbook images — this generator is grounded in curriculum.json only.
    file_map = {}
...
# --- Main Content ---
st.markdown("<h1 class='main-header'>Grade 6-9 Math &amp; English Practice Generator</h1>", unsafe_allow_html=True)
...
# The dropdown Skill Domains are loaded from curriculum.json so they are identical
# to the domains shown on the Placement Test report (English 5 + Math 5). The same
# file grounds the generation prompt (Easy/Medium/Hard mapped to low/mid/high skills).

@st.cache_data
def load_curriculum():
    try:
        with open(Path(__file__).resolve().parent / "curriculum.json", "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception as e:
        st.error(f"curriculum.json load failed: {e}")
        return {}

def curriculum_domain_names(subject):
    cur = load_curriculum()
    return [m.get("name", t) for t, m in cur.items()
            if t != "_meta" and isinstance(m, dict) and m.get("subject") == subject]

def domain_meta_by_name(name):
    cur = load_curriculum()
    for t, m in cur.items():
        if t != "_meta" and isinstance(m, dict) and m.get("name") == name:
            return t, m
    return None, None

def _skill_line(rows):
    parts = []
    for r in rows or []:
        skill = (r.get("skill") or "").strip()
        ex = (r.get("example") or "").strip()
        if ex and len(ex) > 140:
            ex = ex[:140].rstrip() + "..."
        if skill and ex:
            parts.append(f"{skill} (e.g., {ex})")
        elif skill:
            parts.append(skill)
    return "; ".join(parts)

def build_prompt_curriculum_block(domain_name):
    """Curriculum-grounded prompt fragment: maps Easy/Medium/Hard to the domain's
    low/mid/high skills so difficulty aligns with the Placement report's levels."""
    _tag, meta = domain_meta_by_name(domain_name)
    if not meta:
        return ""
    lv = meta.get("levels", {})
    easy, med, hard = _skill_line(lv.get("low")), _skill_line(lv.get("mid")), _skill_line(lv.get("high"))
    lines = ["**CURRICULUM SKILL COVERAGE (Grade 6-9 - MUST FOLLOW):**"]
    if easy: lines.append(f"- [Easy] questions test these FOUNDATIONAL skills: {easy}")
    if med:  lines.append(f"- [Medium] questions test these INTERMEDIATE skills: {med}")
    if hard: lines.append(f"- [Hard] questions test these ADVANCED skills: {hard}")
...
with col_m:
    ...
    if math_sub_options:
        ...
        st.caption("Questions will cover the full skill domain.")
...
"""
                ...
                math_topic_line = f"Create **{total_count} Grade 6-9 Math Practice Questions** for the skill domain: **'{selected_math}'**."
                math_subtopic_focus = build_prompt_curriculum_block(selected_math)
                math_diversity_line = "- Each question MUST test a DIFFERENT sub-skill within this domain, drawn from the curriculum skills listed above."
            ...
            **INSTRUCTIONS:**
            - Target difficulty appropriate for grades 6-9 (US middle school through Algebra 1). Do NOT drift up to SAT-level difficulty.
...
"""
                    ...
                    topic_line = f"Create **{total_count} Grade 6-9 English Practice Questions** for the skill domain: **'{selected_eng}'**."
                    subtopic_focus = build_prompt_curriculum_block(selected_eng)
                    diversity_line = "- Each question MUST test a DIFFERENT sub-skill within this domain, drawn from the curriculum skills listed above."
                ...
                **INSTRUCTIONS:**
                - Target passage length, vocabulary, and question style appropriate for grades 6-9. Do NOT drift up to SAT/DSAT-level difficulty.
    ...
    with r_col2:
        ...
        if docx_data:
            st.download_button(
                ...
                data=docx_data,
                file_name="Elite_Grade6-9_Practice_Set.docx",
