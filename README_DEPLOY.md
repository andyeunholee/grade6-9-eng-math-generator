...
3.  **Repository name**에 `grade6-9-eng-math-generator` (또는 원하는 이름)을 입력합니다.
4.  **Public** (공개) 또는 **Private** (비공개)를 선택합니다. (무료 계정은 Public 추천)
5.  다른 설정(Add a README 등)은 건드리지 말고 맨 아래 **'Create repository'** 버튼을 클릭합니다.
6.  생성 완료 후 나오는 화면에서 **HTTPS 주소** (예: `https://github.com/사용자명/grade6-9-eng-math-generator.git`)를 복사해둡니다.
...
1.  **원격 저장소 연결** (위에서 복사한 주소를 붙여넣으세요)
    ```powershell
    git remote add origin https://github.com/여러분의_아이디/grade6-9-eng-math-generator.git
    ```
...
2.  **'New app'** 버튼을 클릭합니다.
3.  **'Use existing repo'**를 선택하고, 방금 만든 `grade6-9-eng-math-generator` 저장소를 선택합니다.
4.  설정은 다음과 같이 확인합니다:
